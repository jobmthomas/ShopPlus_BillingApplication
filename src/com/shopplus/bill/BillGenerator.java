...
public class BillGenerator {
	Logger logger = Logger.getLogger(BillGenerator.class);
	...
	public Map<Integer, char[][]> generateBill(int billNo, DBUtils dbUtils) {
		...
		try {
			...
			NodeList nodesBillDimentionList = doc.getElementsByTagName("BILL_DIMENTION");
			Node nodesBillDimention = nodesBillDimentionList.item(0);
			Element billDimentionElement = (Element) nodesBillDimention;
				...
				for (int i = 0; i < itemChildNodes.getLength(); i++) {
					...
					if (!itemChildNode.getNodeName().equals("#text")) {
						...
						case "PRINT_CHAR":
							...
							billPage = putValueInBill(billPage, itemLineNoInThePage, position, value, size, align);

							break;

						case "SL_NO":

							billPage = putValueInBill(billPage, itemLineNoInThePage, position, slNo + "", size, align);

							break;
						case "QTY":

							billPage = putValueInBill(billPage, itemLineNoInThePage, position,
						...
						case "EXPD":
							...
							billPage = putValueInBill(billPage, itemLineNoInThePage, position, sdf.format(expd) + "",
									size, align);
							break;
						case "RATE":
						...
						case "DISCOUNT":
							...
							billPage = putValueInBill(billPage, itemLineNoInThePage, position, df.format(discount) + "",
									size, align);
							break;
						...
						case "CGST":
							...
							billPage = putValueInBill(billPage, itemLineNoInThePage, position, df.format(cgst) + "",
									size, align);
							break;
						case "SGST_PER":
						...
						case "SGST":
							...
							billPage = putValueInBill(billPage, itemLineNoInThePage, position, df.format(sgst) + "",
									size, align);
							break;
						case "GST_PER":
						...
						case "GST":
							...
							sgst = (taxableAmount / 100) * billDetail.getsGst();
							gst = cgst + sgst;
							billPage = putValueInBill(billPage, itemLineNoInThePage, position, df.format(gst) + "",
									size, align);
							break;
						...
						case "NET_TOTAL":
							...
							billPage = putValueInBill(billPage, itemLineNoInThePage, position, df.format(netTotal) + "",
									size, align);
							break;
						}
				...
				if (!itemNameToExplode.equals("")) {
					while (true) {
						...
						if (itemNameToExplode.length() > itemNameSize) {
							...
							char[][] billPage = bill.get(pageNo);
							billPage = putValueInBill(billPage, itemLineNoInThePage, itemNamePosition,
	...
	private char[][] printGeneralInfo(char[][] billPage, Document doc, BillMaster billMaster, int billWidth,
		...
		if (nodesHeader.getLength() != 0) {
			...
			for (int i = 0; i < headerChildNodes.getLength(); i++) {
				...
				if (!headerChildNode.getNodeName().equals("#text")) {
					...
					case "CUSTOMER_NAME":
						billPage = putValueInBill(billPage, lineNo, position, billMaster.getCustomerName() + "", size,
								align);
						break;
					case "DR_NAME":
						billPage = putValueInBill(billPage, lineNo, position, billMaster.getDoctorName() + "", size,
								align);
						break;
					case "LISCENCE_NO":
						break;
					case "GSTIN":
						break;
					case "BILL_NO":
						billPage = putValueInBill(billPage, lineNo, position, billMaster.getBillNumber() + "", size,
								align);
						this.billNo = billMaster.getBillNumber();
						break;
					case "DATE":
						...
						billPage = putValueInBill(billPage, lineNo, position, sdf.format(createdDate) + "", size,
								align);
						break;
					case "PRINT_CHAR":
						...
						billPage = putValueInBill(billPage, lineNo, position, value, size, align);
						break;
					...
					case "GRAND_TOTAL":
						...
						billPage = putValueInBill(billPage, lineNo, position, df.format(grandTotal) + "", size, align);
					...
					case "RATE_TOTAL":
						...
						billPage = putValueInBill(billPage, lineNo, position, df.format(rateTotal) + "", size, align);
					...
					case "DISCOUNT_TOTAL":
						...
						billPage = putValueInBill(billPage, lineNo, position, df.format(discountTotal) + "", size,
								align);
					...
					case "EXTRA_DISCOUNT":
						...
						df = new DecimalFormat(decimalFormat);
						float extraDiscount = billMaster.getExtraDiscount();

						billPage = putValueInBill(billPage, lineNo, position, df.format(extraDiscount) + "", size,
								align);
						break;
					case "FINAL_TOTAL":
						...
						extraDiscount = billMaster.getExtraDiscount();
						...
						billPage = putValueInBill(billPage, lineNo, position, df.format(finalTotalRounded) + "", size,
								align);
						break;
					case "GST_TOTAL":
						...
						billPage = putValueInBill(billPage, lineNo, position, df.format(totalGst) + "", size, align);
						break;
					case "CGST_TOTAL":
						...
						billPage = putValueInBill(billPage, lineNo, position, df.format(totalCGst) + "", size, align);
					case "SGST_TOTAL":
						...
						billPage = putValueInBill(billPage, lineNo, position, df.format(totalSGst) + "", size, align);
						break;
					...
					case "TAXABLE_AMOUNT_TOTAL":
						...
						df = new DecimalFormat(decimalFormat);
						billPage = putValueInBill(billPage, lineNo, position, df.format(totalTaxable) + "", size,
								align);
						break;
					}
	...
	public char[][] putValueInBill(char[][] bill, int line, int position, String value, int size, String align) {
